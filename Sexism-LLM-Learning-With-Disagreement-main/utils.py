...
import json
from exist2025evaluation import main
...
def merge_data_labels(package_path, label_gold_path, data_path, dataset):
    ...
    for partition in ['dev', 'training']:
        path_partition = package_path + '/' + partition + '/EXIST2025_' + partition + '.json'
...
def test_to_csv(package_path, data_path, dataset):
    partition = 'test'
    path = package_path + '/' + partition + '/EXIST2025_' + partition + '_clean' + '.json'
...
def eval_preds(task, data_train, data_val, epoch, transformer):
    ...
    if data_val == 'dev':
        path_gold_file = config.LABEL_GOLD_PATH + '/' + 'EXIST2025_' + data_val + '_' + task + '_gold_soft.json'
    else:
        path_gold_file = config.LABEL_GOLD_PATH + '/' + 'EXIST2025_' + data_train + '_' + task + '_gold_soft.json'
    
    # 检查文件是否存在
    if not os.path.exists(path_json_results):
        print(f"错误: 预测结果文件不存在: {path_json_results}")
        return 0.0
    
    if not os.path.exists(path_gold_file):
        print(f"错误: 金标准文件不存在: {path_gold_file}")
        return 0.0
    ...
    try:
        ...
        with open(path_json_results, 'r') as f:
            ...
            if not results or not isinstance(results, dict):
                print(f"错误: 预测结果文件格式不正确")
                return 0.0
                
        with open(path_gold_file, 'r') as f:
            gold = json.load(f)
            if not gold or not isinstance(gold, dict):
                print(f"错误: 金标准文件格式不正确")
                return 0.0

        print(f"开始评估:")
        print(f"- 预测文件: {path_json_results}")
        print(f"- 金标准文件: {path_gold_file}")
        print(f"- 任务: {task}")
        
        result_icm_soft_soft = main(['-p', path_json_results, '-g', path_gold_file, '-t', task])
        
        if result_icm_soft_soft is None:
            print(f"警告: 评估返回None值")
            return 0.0
